...
const controlAddRecipe = async function (newRecipe) {
  try {
    ...
    // Change ID in URL
    window.history.pushState(null, '', `#${model.state.recipe.id}`); // В HTML документе метод history.pushState() добавляет новое состояние в историю браузера. SYNTAX: history.pushState(state, title[, url])
    // window.history.back(); // вернуться на последнюю строку
