...
export const uploadRecipe = async function (newRecipe) {
  ...
  const ingredients = Object.entries(newRecipe)
    .filter(entry => entry[0].startsWith('ingredient') && entry[1] !== '') // выводим только эл-ты с названием ingredient и не с пустым значением.
    .map(ing => {
      const [quantity, unit, description] = ing.replaceAll(' ', '').split(','); // Деструктурируем, удаляя пробелы
